...
var TEST_STATUS = {
  ...
  slack: {
    ...
    targetUsers: OPTS.SLACK.TARGET_USERS.CHANNEL,
    messageTemplates: [
      '{emoji} {userTags} {userFullName} marked {numMarked} item{plural} for {projectName} as *test* by Student Government.'
    ],
 ...
 */
function buildItemListSlackAttachment(items, projectName, projectSheetUrl, user, action) {
  var attachment = {
    type: "button",
    text: "List Items",
  ...
  Object.getOwnPropertyNames(itemsByCategory).forEach(function(category) {
    var categoryAttachment = {
      ...
      fields: itemsByCategory[category].map(function(item) {
        var itemField = {
          title: truncateString(item.quantity + "× " + item.name, 85),
          value: "$" + item.totalPrice.toFixed(2),
          short: "false"
        }

        if(item.requestorComments) itemField.value += "\n Requestor Note: _" + item.requestorComments + "_";
        if(item.officerComments) itemField.value += "\n Officer Note: _" + item.officerComments + "_";     
