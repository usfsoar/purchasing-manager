 ...
 * @prop {string[]} allowedPrevious Allowed previous statuses as their text
 * properties.
 ...
 * @prop {?string} actionText.fastForward Menu item text for fast-forwarding
 * items.
 ...
 * sheet.
 * @prop {(() => string)[]} slack.channelWebhooks Webhooks to send Slack
 * messages to.
 ...
 * @prop {?(Column[])} fastForwardColumns.user Column to input attribution email
 * address into.
 * @prop {?(Column[])} fastForwardColumns.date Column to input action date into.
 * @prop {?(Column[])} requiredColumns Optional required columns needed to
 * perform actions.
 * @prop {?(Column[])} reccomendedColumns Optional reccomended columns desired
 * to perform actions.
 ...
 * Slack).
 * @param {unknown} e The request body.
 * @return {void}
 */
function doPost(e) {
  let message = {
 ...
 * @return {string[]} Filled in message strings.
 */
 ...
 * @return {Object} A valid Slack message with attachments.
 */
    ...
    attachments: [
      {
        fallback:
          ...
          '%) remaining, out of a total annual budget of $' +
 ...
 * @return {string[]} Unordered array of values, flattened into a 1-dimensional
 * array.
 */
...
function buildAndAddCustomMenu() {
  ...
  customMenu
      ...
      .addItem(
          ...
          markSelectedReceived.name
      )
      .addSeparator()
      .addItem(
          STATUSES_DATA.RECEIVED_REIMBURSE.actionText.selected,
          markSelectedReceivedReimburse.name
      );
 ...
 * @returns {{slackId:string,fullName:string,email:string,isFinancialOfficer:
 * boolean,phone?:string}}
 ...
 * @return {boolean} true if the user is a financial officer.
 */
 ...
 * @return {?string} The Slack ID or null if no match.
 */
 ...
 * @param {string} email Email address of the person to look for.
 * @return {string} The Slack ID or '' if no match.
 */
 ...
 * @param {string} [conjunction='and'] Conjunction to use at the end of the
 * list.
 * @param {boolean} [noOxfordComma] If true, won't add an Oxford comma.
 * @return {string} A nicely formatted list, ie: 'One, Two, and Three'.
 */
function makeListFromArray(listArray, conjunction, noOxfordComma) {
  /**
   * The oxford comma, or an empty string if `noOxfordComma` is true or the
   * array is too short.
   */
  const oxfordComma = noOxfordComma || listArray.length <= 2 ? '' : ',';
 ...
 * @return {GoogleAppsScript.Spreadsheet.Range[]} Array of selected ranges,
 * expanded to cover entire width of data in the sheet.
 */
function getSelectedRows() {
  const activeSheet = SpreadsheetApp.getActiveSheet();
  const selections = activeSheet
      .getSelection()
      .getActiveRangeList()
      .getRanges();
 ...
 * @return {GoogleAppsScript.Spreadsheet.Range[]} Array with one Range.
 */
function getAllRows() {
  const activeSheet = SpreadsheetApp.getActiveSheet();
 ...
 * @param {?string} sheetName Name of the sheet to check. If empty, uses current
 * sheet.
 * @return {boolean} True if a project sheet is active.
 */
 ...
 * @return {GoogleAppsScript.Spreadsheet.Range} The range of the column.
 */
...
function markItems(newStatus, markAll) {
  ...
  const itemRequestors = /** @type {string[]} */ ([]);
 ...
 * @param {Status} newStatus The object representing the status to fast-forward
...
function fastForwardItems(newStatus) {
  ...
  const pastUserColumns = newStatus.fastForwardColumns.user.map((ffCol) =>
    getColumnRange(ffCol.index)
  );
  const pastDateColumns = newStatus.fastForwardColumns.date.map((ffCol) =>
    getColumnRange(ffCol.index)
  );
...
function isCurrentStatusAllowed(currentStatusText, newStatus) {
  const currentStatusTrimmed = currentStatusText.trim();
 ...
 * @return {?string} String, unless nullIfMissing is specified.
 */
